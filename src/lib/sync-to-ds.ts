...
export const upsertEmail = async (
  email: EmailMessage,
  accountId: string,
  index: number,
) => {
  ...
  try {
    ...
    // insert the senders and receivers of the mail into EmailAddress table but if it is already
    // present then update its receivedTo, receivedCc, etc. fields
    ...
    for (const address of [
      ...
      addressesToUpsert.set(address.address, address); // mapping from address to EmailAddress object
    }
